                ...
                <div className="space-y-5 md:space-y-0 md:grid md:grid-cols-2 md:gap-6 items-end">
                  ...
                  <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl flex flex-col justify-center">
                    ...
                    <div className="flex items-center space-x-2 text-blue-800 font-bold text-sm">
                      ...
                      <span>{getApprovalRoute()}</span>
                    </div>
                  </div>
                </div>
                <div className="pt-4">
                  <button className="w-full border-2 border-dashed border-blue-400 rounded-2xl p-6 flex flex-col items-center justify-center bg-blue-50 hover:bg-blue-100 transition-colors">
                    <div className="flex space-x-4 mb-2">
                      <Camera className="w-8 h-8 text-blue-500" />
                      <UploadCloud className="w-8 h-8 text-blue-500" />
                    </div>
                    <span className="text-blue-700 font-bold text-sm">撮影 / ファイルを選択</span>
                  </button>
                </div>
                <button onClick={() => setShowDemoAlert(true)} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-lg mt-6 flex items-center justify-center space-x-2 text-lg">
          ...
          <main className="p-6 md:p-8">
            {approverTab === 'list' && (
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                <h3 className="font-bold text-gray-900 text-lg">山田 太郎 (A保育園)</h3>
                <p className="text-sm text-gray-600 mb-4 bg-gray-50 p-3 rounded-lg italic">「粘土・画用紙の補充」</p>
                <div className="text-3xl font-bold text-indigo-700 mb-6">¥12,800</div>
                <div className="flex space-x-3">
                  <button onClick={() => setRejectModal({isOpen: true, itemId: 1})} className="flex-1 py-3 bg-gray-100 text-gray-600 rounded-xl font-bold text-sm">差し戻し</button>
                  <button onClick={() => alert('承認しました')} className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold text-sm shadow-md">承認する</button>
                </div>
              </div>
            )}
            {approverTab === 'report' && (
